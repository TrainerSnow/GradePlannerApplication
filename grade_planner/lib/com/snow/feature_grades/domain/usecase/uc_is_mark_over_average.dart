...
class IsOverAverage {
  ...
  Future<bool> callGrade(Grade grade, int orderMode) async {
    ...
    var result = orderMode == 1 ? grade.value <= meanAvg : grade.value >= meanAvg;

    return result;
  }

  Future<bool> callSubject(Subject grade, int orderMode) async {
    return orderMode == 1 ? grade.average() <= await _getMeanAverage.call() : grade.average() >= await _getMeanAverage.call();
  }
}
